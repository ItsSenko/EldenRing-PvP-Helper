...
namespace PvPHelper.Core
{
    public class CustomPointers
    {
        ...
        private const string GetEquipParamGoodsFuncAOB = "?? ?? ?? ?? ?? 40 48 c7 44 24 20 fe ff ff ff 48 89 5c 24 50 48 89 6c 24 58 48 89 74 24 60 8b fa 48 8b f1 33 db 85 d2 ?? ?? ?? 01 00 00 ?? ?? ?? ?? ?? 04 03 48 85 c9 75 2e ?? ?? ?? ?? ?? 02 03 ?? ?? ?? 18 01 4c 8b c8 ?? ?? ?? ?? ?? 55 02 ba b4 00 00 00 ?? ?? ?? ?? ?? c8 01 ?? ?? ?? 17 01 48 8b 0d ?? ?? ?? ?? 45 ?? ?? 41 8d ?? 03";

...
namespace ERPvPHelper
{
    ...
    public class BuildSaver
    {
        ...
        {
            string FilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), $"Elden Ring PvPHelper/Builds/{build.BuildName}.json");
            string json = JsonConvert.SerializeObject(build, Formatting.Indented);
            if (File.Exists(FilePath))
                File.Delete(FilePath);
            using (StreamWriter sw = new StreamWriter(FilePath))
        ...
        public static List<BuildPrefab> getBuilds()
        {
            ...
            string FilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), $"Elden Ring PvPHelper/Builds/");
